...
pub async fn run(
    ...
    if liveness::is_app_running(config)? {
        println!("Factory Desktop is running. Close it before rollback.");
        return Ok(());
    }
...
async fn trigger_rollback(
    ...
    if status.success() {
        ...
        println!(
            "Rolled back Factory Desktop to {}.",
...
mod tests {
    ...
    fn records_existing_current_package_as_known_good() -> Result<()> {
        ...
        let package_path = temp.path().join("factory.deb");
        ...
        assert_eq!(state.last_known_good_version.as_deref(), Some("0.108.0"));
        assert_eq!(
    ...
    fn ignores_missing_current_package() {
        let mut state = PersistedState::new(true);
        state.installed_version = "0.108.0".to_string();
        state.artifact_paths.package_path = Some(std::path::PathBuf::from("/missing/factory.deb"));
    ...
    fn successful_rollback_repoints_package_paths_to_installed_package() -> Result<()> {
        let temp = tempfile::tempdir()?;
        let update_path = temp.path().join("candidate.deb");
        let rollback_path = temp.path().join("known-good.deb");
        std::fs::write(&update_path, b"new")?;
        std::fs::write(&rollback_path, b"old")?;

        let mut state = PersistedState::new(true);
        state.installed_version = "0.109.0".to_string();
        state.candidate_version = Some("0.109.0".to_string());
        state.status = UpdateStatus::Installing;
        state.artifact_paths = ArtifactPaths {
            dmg_path: None,
            workspace_dir: Some(temp.path().join("workspaces/0.109.0")),
        ...
        assert_eq!(state.last_known_good_version.as_deref(), Some("0.107.0"));
        assert_eq!(
